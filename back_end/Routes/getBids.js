    ...
    if (req.query.type)
        switch (req.query.type) {
            case 'auction':
                ...
                const auction = await AuctionModel.findById(req.query.auctionId).populate('proposals');
                if (auction) {
            ...
            case 'owner':
                const proposals = await proposalModel.find({ ownerId: req.user._id }).populate('auctionId');
                if (req.query.status) {
                    let temp = [];
